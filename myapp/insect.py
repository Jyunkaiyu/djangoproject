    ...
    def scrape(self):
        ...
        if self.name:  # 如果城市名稱非空值
            ...
            response = requests.get(
                f"https://shopee.tw/search?keyword={self.name}",headers=headers)
            soup = BeautifulSoup(response.text, "lxml")
 
            # 取得十個票券卡片(Card)元素
            activities = soup.find_all(
                "div", {"class","row shopee-search-item-result__items"}, limit=10)
            
            if activities : print(123)
            else:print("不存在")
            ...
            for activity in activities:
                ...
                if title : title = activity.find(
                    "div", {"class": "ie3A+n bM+7UW Cve6sh"}).get_text().strip()
                else:print('元素不存在')
 
                # 票券價格
                price = activity.find(
                    "div", {"class": "vioxXd rVLWG6"})
                if price : price = activity.find(
                    "div", {"class": "vioxXd rVLWG6"}).get_text().strip()
                else:print('元素不存在')
                ...
                result.append(
                    dict(title=title, link=link, price=price, booking_date=booking_date, star=star, source="https://buzzorange.com/techorange/2018/02/09/makes-pchome-great-again/"))
