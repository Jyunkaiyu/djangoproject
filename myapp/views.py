...
#from djangoproject.myapp.cpurank import cpuchallenge
...
from .insect import  pchome
from .cpurank import cpuchallenge
from .gpurank import gpuchallenge
...
def cpurank(request):
    Cpu = cpuchallenge(request.POST.get("cpu_name1"),request.POST.get("cpu_name2"))

    context = {
        "cpuranks":Cpu.scrape()
    }

    return render(request, "card_comparative/cpu_card.html", context)
...
def gpurank(request):
    ...
    return render(request, "card_comparative/gpu_card.html", context)
